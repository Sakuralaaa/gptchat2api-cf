...
func (w *registerWorker) ua() string {
...
// prewarmCloudflare uses a FlareSolverr instance to pass the Cloudflare challenge
// guarding auth.openai.com. The returned cf_clearance cookie is injected into the
// worker cookie jar and the browser User-Agent is adopted for every later request,
// because cf_clearance is bound to the exact User-Agent (and exit IP) that solved it.
func (w *registerWorker) prewarmCloudflare(ctx context.Context) error {
	endpoint := strings.TrimSpace(util.Clean(w.config["flaresolverr"]))
	if endpoint == "" {
		return nil
	}
...
func normalizeRegisterConfig(raw map[string]any) map[string]any {
	...
	cfg["flaresolverr"] = strings.TrimSpace(util.Clean(cfg["flaresolverr"]))
