...
export function RegisterCard() {
  ...
  const setFlaresolverr = useSettingsStore((state) => state.setRegisterFlaresolverr);
    ...
    <div className="grid h-[calc(100vh-132px)] min-h-[640px] items-stretch gap-0 overflow-hidden rounded-[24px] border border-[#f2f3f5] bg-card shadow-[0_0_15px_rgba(44,30,116,0.16)] xl:grid-cols-2">
      <section className="space-y-4 overflow-y-auto border-b border-border p-4 xl:border-r xl:border-b-0">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="space-y-2">
              <label className="text-sm text-stone-700">FlareSolverr 地址</label>
              <Input value={config.flaresolverr ?? ""} onChange={(event) => setFlaresolverr(event.target.value)} placeholder="http://127.0.0.1:8191" className="h-10 rounded-xl border-stone-200 bg-white" disabled={config.enabled} />
            </div>
            <div className="space-y-2">
